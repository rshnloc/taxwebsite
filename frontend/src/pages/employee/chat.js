  ...
  return (
    <DashboardLayout>
      <div className="space-y-4">
        <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Client Messages</h1>

        {chatRooms.length === 0 ? (
          <EmptyState icon={MessageCircle} title="No conversations yet" description="Chat rooms will appear when you're assigned to applications" />
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4" style={{ height: '600px' }}>
            <div className="lg:col-span-1 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-y-auto">
              <div className="p-3 border-b border-slate-200 dark:border-slate-700">
                <h2 className="font-semibold text-slate-900 dark:text-white text-sm">Conversations ({chatRooms.length})</h2>
              </div>
              {chatRooms.map(room => {
